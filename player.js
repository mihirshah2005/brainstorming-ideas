...
function onInteractKey(){
  if (!started || endingActive) return;
  if (window.SHIP && SHIP.active) return;
  if (window.SHIP && !SHIP.active && !dialogOpen && SHIP.pos.y<5 &&
      Math.hypot(player.x-SHIP.pos.x, player.z-SHIP.pos.z)<6) return; // ship.js boards
